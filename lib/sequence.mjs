  ...
  const genKeyframe = async (i, force = false) => {
    ...
    if (!force && versionsOf(outDir, prefix, "seq", n, ".png", seq[i].title).length) {
      ...
      if (fs.existsSync(dest)) {
        ...
        // Anchor every scene image on the generated reference visual: upload
        // the selected main ref version and run Flux img2img from it, so the
        // reference pixels actually condition the keyframe (previously the
        // ref image was generated and then never used). Falls back to pure
        // text-to-image when no reference exists yet.
        const refFile = resolveMain(outDir, prefix, "ref", 0, ".png", null, loadState(outDir));
        let refInput = null;
        if (refFile) {
          refInput = await uploadToInput(path.join(outDir, refFile), `${prefix}_ref${n}`);
        } else {
          console.log(`${tag} keyframe ${n}/${N} — no reference yet, text-to-image only`);
        }
        console.log(`${tag} keyframe ${n}/${N} (${seq[i].title}, v${v})${refInput ? ` from ${refFile}` : ""}...`);
