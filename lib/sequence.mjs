...
import {
  ...
  nextFinalVersion, getKeyframeRefFrom, setKeyframeRefFrom,
...
export async function runSequence({
  ...
  const genRef = async (force = false) => {
    ...
    const mainFile = newFile ?? path.basename(resolveMain(outDir, prefix, "ref", 0, ".png", null, loadState(outDir)));
    setMain(outDir, prefix, "ref", 0, null, mainFile);
  ...
  const genKeyframe = async (i, force = false) => {
    ...
    if (!force && !stale && versionsOf(outDir, prefix, "seq", n, ".png", seq[i].title).length) {
      ...
      if (!stale && !force && fs.existsSync(dest)) {
        ...
        const entry = await run(buildKeyframe(seq[i].image, i, refInput));
        await download(firstImageUrl(entry), dest);
        generated = true;
        newFile = path.basename(dest);
      }
    ...
    // v1/v2/v3 on disk selects v4); skips keep the current selection.
    const mainFile = newFile ?? path.basename(resolveMain(outDir, prefix, "seq", n, ".png", seq[i].title, loadState(outDir)));
    setMain(outDir, prefix, "seq", n, seq[i].title, mainFile);
  ...
  const genClip = async (i, force = false) => {
    ...
    // stitch uses it); skips keep the current selection.
    const mainFile = newFile ?? path.basename(resolveMain(outDir, prefix, "clip", n, ".mp4", seq[i].title, loadState(outDir)));
    setMain(outDir, prefix, "clip", n, seq[i].title, mainFile);
