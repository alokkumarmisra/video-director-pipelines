...
export function buildScenesPrompt({ input, blueprint, beats, prevScene, startNumber, count, styleLock }) {
  const { chars, locs, objs } = identityContext(blueprint);
  const beatLines = beats.map((b) => `Beat ${b.n}: ${b.title} — ${b.summary}`).join("\n");
  return [
    `TASK 2/2 — SCENE PLAN (scenes ${startNumber}..${startNumber + count - 1} of the movie).`,
