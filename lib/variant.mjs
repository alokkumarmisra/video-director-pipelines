...
// Immutable storage slugs — the single source of truth for every on-disk
// name and every path saved in the database. Display names may contain
// spaces/caps; storage names never do (lowercase, underscore-separated).
// projects.folder_name is set from the project name ONCE at creation and is
// never updated on edits/renames, so output dirs, filenames and DB paths
// stay stable and media keeps resolving after a rename.
/** Output folder base for a project display name (<=100 chars). */
export const folderSlug = (s) =>
  String(s ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 100) || "project";
/** Filename fragment for a beat title (<=60 chars). */
export const fileSlug = (s) =>
  String(s ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 60) || "scene";
