...
/**
 * Parse a `SELECT max(version)` cell into a real version number, or null.
 * `max()` yields NULL when the project has zero version rows — and
 * `Number(null)` is 0, which is a valid integer but NEVER a real version
 * (`project_assets` has CHECK (version > 0)). Routing that phantom 0 into
 * an asset INSERT is what produced "new row violates check constraint
 * project_assets_version_check". Every max(version) read that feeds a
 * project_assets write must go through this helper (null = no versions
 * yet -> mint v1 via the delta path, never upsert at v0).
 */
export function latestVersionOf(v) {
