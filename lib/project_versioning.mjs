...
// asset_type) with version <= V (PostgreSQL DISTINCT ON, see
...
// (project_id, version, asset_type, beat_index) constraint, which already
// supports deltas (one row per changed asset per version).
...
export function resolveEffective(rows, version) {
  const best = new Map(); // `${beat_index}:${asset_type}` -> row
  for (const r of rows || []) {
    if (Number(r.version) > Number(version)) continue;
    const k = `${r.beat_index}:${r.asset_type}`;
...
/**
 * Effective assets at a version: latest applicable row per
 * (beat_index, asset_type) with version <= $2. DISTINCT ON requires the
 * leading ORDER BY columns to match the DISTINCT ON list.
 */
...
/** Exact rows stored at one version (delta contents / version history). */
export const EXACT_VERSION_SQL = `SELECT id, project_id, version, scene_id, beat_index, beat_title,
       asset_type, status, prompt, negative_prompt, file_path,
       model, workflow, seed, attempts, max_retries, error_message,
       metadata, started_at, completed_at, created_at, updated_at
  FROM project_assets WHERE project_id = $1 AND version = $2
